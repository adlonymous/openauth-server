/**
 * Custom PasswordUI with resend code button on registration verification.
 * 
 * This extends the default PasswordUI to add a "Resend code" button
 * during the registration email verification step.
 */
...
/**
 * Custom PasswordUI with resend code on registration.
 */
    ...
    register: async (_req, state, form, error): Promise<Response> => {
      ...
      const jsx = (
        <Layout>
          ...
          {/* ADDED: Resend code button for registration verification */}
          {state.type === "code" && (
            <form method="post">
              {/* action="register" with state.type="code" triggers resend logic in PasswordProvider */}
              <input type="hidden" name="action" value="register" />
              <div data-component="form-footer">
                <span>
                  {copy.code_return}{" "}
